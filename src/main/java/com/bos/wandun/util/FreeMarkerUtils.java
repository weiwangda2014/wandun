...
import com.bos.wandun.CommonAttributes;
import com.bos.wandun.EnumConverter;
...
public final class FreeMarkerUtils {
	...
	/** BeansWrapper */
	private static final BeansWrapper DEFAULT_BEANS_WRAPPER = new BeansWrapperBuilder(Configuration.VERSION_2_3_22)
			.build();
	...
	public static String process(String template, Map<String, ?> model, Configuration configuration)
		...
		new Template("template", new StringReader(template),
				configuration != null ? configuration : DEFAULT_CONFIGURATION).process(model, out);
	...
	public static <T> T getParameter(String name, Class<T> type, Map<String, TemplateModel> params)
		...
		Assert.hasText(name, SpringUtils.getMessage("assert.validation.constraints.hasText.message"));
		Assert.notNull(type, SpringUtils.getMessage("assert.validation.constraints.notNull.message"));
		Assert.notNull(params, SpringUtils.getMessage("assert.validation.constraints.notNull.message"));
	...
	public static <T> T getArgument(int index, Class<T> type, List<?> arguments) throws TemplateModelException {
		Assert.notNull(type, SpringUtils.getMessage("assert.validation.constraints.notNull.message"));
		Assert.notNull(arguments, SpringUtils.getMessage("assert.validation.constraints.notNull.message"));
	...
	public static TemplateModel getVariable(String name, Environment env) throws TemplateModelException {
		Assert.hasText(name, SpringUtils.getMessage("assert.validation.constraints.hasText.message"));
		Assert.notNull(env, SpringUtils.getMessage("assert.validation.constraints.notNull.message"));
	...
	public static void setVariables(Map<String, Object> variables, Environment env) throws TemplateException {
		Assert.notNull(variables, SpringUtils.getMessage("assert.validation.constraints.notNull.message"));
		Assert.notNull(env, SpringUtils.getMessage("assert.validation.constraints.notNull.message"));
