...
import com.bos.wandun.entity.BaseOrderEntity;
 ...
 * @version 4.0
 */
public abstract class BaseDaoImpl<T extends BaseEntity<ID, T>, ID extends Serializable> implements BaseDao<T, ID> {
     ...
     * 别名前缀
     */
    private static final String ALIAS_PREFIX = "bosGeneratedAlias";
    ...
    protected List<T> findList(CriteriaQuery<T> criteriaQuery, Integer first, Integer count, List<Filter> filters, List<Order> orders) {
        ...
        if (orderList.isEmpty()) {
            if (BaseOrderEntity.class.isAssignableFrom(entityClass)) {
                ...
                orderList.add(criteriaBuilder.desc(getPath(root, BaseOrderEntity.CREATE_DATE_PROPERTY_NAME)));
            }
        }
